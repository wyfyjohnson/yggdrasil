...
from libqtile import bar, hook, layout, qtile, widget  # backend
...
keys = [
    ...
    Key(
        [mod],
        "end",
        lazy.spawn("rofi -show drun -theme ~/.config/waybar/rofi/Launcher.rasi"),
    ),
    Key([mod], "d", lazy.spawn("webcord")),
...
screens = [
    Screen(
        top=bar.Bar(widgets=widgets_list, size=30, background=colors[0], margin=6),
    ),
    Screen(
        top=bar.Bar(
            widgets=secondary_widgets_list, size=30, background=colors[0], margin=6
        ),
    ),
]
...
wl_output_rules = {
    ...
    "DP-1": {"mode": {"width": 3840, "height": 2160}, "pos": {"x": 1080, "y": 0}},
}
